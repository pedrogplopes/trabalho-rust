...
mod biblioteca {
    ...
    impl Biblioteca {
        ...
        #[ink(message)]
        pub fn criar_livro(&mut self, titulo: String, autor: String, data_publicacao: u64, genero: Genero) -> Result<(), String> {
            if titulo.trim().is_empty() || autor.trim().is_empty() {
                return Err("Título e autor não podem ser vazios".to_string());
            }

            let id = self.proximo_id_livro;
            let livro = Livro { id, titulo, autor, data_publicacao, genero };
        ...
        #[ink(message)]
        pub fn emprestar_livro(&mut self, id_livro: u32, usuario: AccountId) -> Result<(), String> {
        ...
        pub fn devolver_livro(&mut self, id_emprestimo: u32) -> Result<(), String> {
            let mut emprestimo = self.emprestimos.get(id_emprestimo).ok_or("Empréstimo não encontrado")?;
